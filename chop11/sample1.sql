...
scott IDENTIFIED BY Oracle12345678;
--RESOURCE : 객체

ALTER USER scott DEFAULT TABLESPACE users;
ALTER USER scott TEMPORARY TABLESPACE temp;
ALTER USER scott IDENTIFIED BY Oracle123; --암호시 특수문자 사용하게 되면 오류가 발생 

GRANT create view, create synonym, create sequence TO scott;
-- REVOKE ... FROM hr;
GRANT SELECT ON emp TO hr;
GRANT select,insert,delete,update ON emp TO hr;

ALTER USER scott ACCOUNT LOCK;			--지정된 계정 잠그기(lock)
ALTER USER scott ACCOUNT UNLOCK;		--지정된 계정 잠금해제(nulock)
ALTER USER scott IDENTIFIED BY Oracle123 
                 ACCOUNT UNLOCK;        --여러명령문 한번에 수행 가능(비밀번호변경, 계정잠금해제)
