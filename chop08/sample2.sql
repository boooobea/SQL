...
-- 제약조건(Contraints)
-- ------------------------------------------------------
-- 가. 테이블에 부적절한 데이터가 저장 되는것을 방지
-- 나. 테이블 생성시, 각 컬럼에 대해서 정의하는 규칙 
-- 다. DB설계단계에서, 데이터의 무결성을 보장하기 위한 수단 
-- 라. 예: 성별(남/여), 부서(NULL값없어야함)
-- 마. 데이터가 저장되기 전에, 무결성을 검사하여 잘못된 데이터 저장을 방지 
-- 바. 테이블에 행이 삽입(INSERT)/수정(UPDATE)/삭제(DELETE)될 때마다 적용
-- 사. 필요 시, 제약조건의 기능을 일시적으로 활성화(Enable)/비활성화(Disable) 가능
...
--  );
-- ------------------------------------------------------
-- 컬럼 레벨(colums-level) 방식의 기본 키(PRIMARY KEY)설정 

CREATE TABLE department (
    deptno  NUMBER(2)   CONSTRAINTS department_deptno_pk PRIMARY KEY,
    dname   VARCHAR2(15),
    loc     VARCHAR2(15)
); 
...
----------------------------------------------------------
-- USER_CONSTRAINTS : 현재 계정에서 조회가능한 딕셔너리
-- ALL_CONSTRAINTS : 모든계정에서 조회가능한 딕셔너리
-- DBA_CONSTRAINTS : SYS 계정으로만 조회가능한 딕셔너리 