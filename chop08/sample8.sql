...
CREATE TABLE emp03 (
    ...
    CONSTRAINT emp03_detpno_fk FOREIGN KEY(deptno) REFERENCES dept02(deptno)
);
...
--ORA-02291: integrity constraint (SCOTT.EMP03_DETPNO_FK) violated - parent key not found
INSERT INTO emp03(empno, ename, deptno)
VALUES (4000,'Mike',50);
...
-- 참조 무결성 제약조건 위배
-- ------------------------------------------------------
-- 특정부서(부모)를 참조하고 있는 사원(자식)들이 있으므로 부서를 삭제할 수 없음
-- (참조관계)
--ORA-02292: integrity constraint (SCOTT.EMP03_DETPNO_FK) violated - child record found
DELETE FROM dept02 
WHERE deptno=10;


--------------------------------------------------------
-- 해결방법 1) ON DELETE CASCADE
-- 참조하는 부모테이블의 행이 삭제되면, 해당 행을 참조하는 자식테이블의 행도 삭제
-- 연쇄삭제(같이삭제된다)
...
--------------------------------------------------------
-- 해결방법 2) ON DELETE SET NULL
-- 참조하는 부모테이블의 행이 삭제되면, 해당 행을 참조하는 자식테이블의 
-- 컬럼값을 NULL로 설정한다.
