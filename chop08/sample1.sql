...
--------------------------------------
-- 데이터베이스 구조(=DB 객체)를 생성/수정/삭제 하는 SQL문장 
-- 가. 자동으로 COMMIT된다(DB에 자동 영구 반영된다)
-- 나. 데이터 딕셔너리(Data Dictionary)에 정보저장 == 데이터 카탈로그
--------------------------------------
-- 다. 오라클 객체(=데이터베이스)의 종류 5가지
-- (1) 테이블(Table) - 데이터 저장
--      기본적인 데이터 저장단위, 행과 열로 구성된 객체
-- (2) 인덱스(Index) - 데이터 검색성능 향상
--      테이블에 저장된 데이터의 검색 성능 향상 목적을 위한 객체
-- (3) 뷰(View) - 가상의 테이블
--      한개 이상의 테이블의 논리적인 부분 집합을 표시할 수 있는 객체
-- (4) 시퀀스(Sequence) - 순차번호 생성시(인조키,대리키)
...
--      컬럼명 데이터 타입 [DEFAULT 값|제약조건] [
--      ...]
--  );
-- 사용자계정 -> 계정 스키마 == Owner.테이블명
--------------------------------------
-- 스키마 
-- 가. 사용자가 DB에 접근하여, 생성한 객체들의 대표이름
-- 나. default, 사용자 계정명과 동일하게 부여 된다.
-- 다. 생성한 객체들의 소유자는 해당 객체를 생성한 사용자 계정 
-- 라. 다른 스키마에 속한 객체 접근은 기본적으로 불가(접근권한필요)
-- 마. 다른 스키마 내의 객체에 대한 접근권한이 있다면, "스키마.객체"형식으로 사용
...
--  (3) NVARCHAR2(size) - 가변길이 문자저장 (1<= size <= 4000, byte), 다국어문자열 
...
-- 테이블 내의 특정 행을 ROWID값으로 빠르게 조회 가능
-- Index를 통하여 간접적으로 ROWID 사용한다. 

SELECT *
FROM dept
WHERE ROWID = 'AAASMyAAAAAACVHAAA'; -- ROWID를 이용한 조건 검색  
...
CREATE TABLE scott.employee(
    ...
    ename VARCHAR2(20),
    hiredate DATE,
    sal NUMBER(7,2)     -- 7->1234.56 : 소수점까지 포함(정수4.소수점2)
);      --employee테이블 생성
...
-- Default 옵션
-- ------------------------------------------------------
-- 가. 테이블에 데이터 저장 시, 특정 컬럼에 값을 지정하지 않으면 자동으로 NULL 저장 
...
VALUES (20,'박지성',4000);      --hiredate컬럼 제외 
