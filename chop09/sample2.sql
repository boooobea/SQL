...
-- ------------------------------------------------------
-- 부서번호 자동생성 시퀀스 객체 생성
CREATE SEQUENCE dept_detno_seq0;

CREATE SEQUENCE dept_detno_seq
    START WITH 10       --10부터 시작
    INCREMENT BY 10     --증가치 10
    MAXVALUE 100        --최대값
    MINVALUE 5          --최소값(CYCLE시, 재시작 번호)
    CYCLE               --최대값 도달시, MINVALUE부터 재시작
    NOCACHE;            --최대 20개의 숫자를 메모리에 미리 생성하지 않음

--DESCRIBE SEQUENCE is not available
DESC dept_detno_seq; --테이블 객체에만 사용가능
...
SELECT
    dept_detno_seq.NEXTVAL,    
    dept_detno_seq.CURRVAL
FROM
    dual;
